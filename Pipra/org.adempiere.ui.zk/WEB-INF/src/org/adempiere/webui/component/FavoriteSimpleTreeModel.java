	...
	public void onEvent(Event event) throws Exception {
		...
		if (Events.ON_DROP.equals(eventName) || Events.ON_RIGHT_CLICK.equals(eventName)
				|| (Events.ON_CLICK.equals(eventName) && comp instanceof Toolbarbutton
						&& ((Toolbarbutton) event.getTarget())
								.getAttribute(FavoriteSimpleTreeModel.MOBILE_TOOLBAR_CTX_MENU) != null
						&& (boolean) ((Toolbarbutton) event.getTarget())
								.getAttribute(FavoriteSimpleTreeModel.MOBILE_TOOLBAR_CTX_MENU))) {
...
//		if(StringUtils.equalsIgnoreCase(mt.toString(),"Sales Order"))
//			return "z-icon-file-text-o ";
//		else if(StringUtils.equalsIgnoreCase(mt.toString(),"Product"))
//			return "z-icon-cube ";
//		else if(StringUtils.equalsIgnoreCase(mt.toString(),"Business Partner"))
//			return "z-icon-handshake-o";
//		else if(StringUtils.equalsIgnoreCase(mt.toString(),"Payment"))
//			return "z-icon-credit-card";
//		else 
		if (mt.isSummary())
			// return "z-icon-Folder";
			return "wms-summary-icon";
		if (mt.isWindow())
//			return "z-icon-Window";
			return "wms-window-icon";
		if (mt.isReport())
			// return "z-icon-Report";
			return "wms-report-icon";
		if (mt.isProcess())
			// return "z-icon-Process";
			return "wms-process-icon";
		if (mt.isTask())
			// return "z-icon-Task";
			return "wms-task-icon";
		if (mt.isWorkFlow())
			// return "z-icon-WorkFlow";
			return "wms-workflow-icon";
		if (mt.isForm())
//			return "z-icon-Form";
			return "wms-form-icon";
		if (mt.isInfo())
			// return "z-icon-Info";
			return "wms-info-icon";
//		return "z-icon-Window";
		return "wms-window-icon";
	} // getIconSclass
