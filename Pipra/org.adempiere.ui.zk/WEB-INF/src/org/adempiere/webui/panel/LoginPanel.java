...
import org.adempiere.webui.component.WmsLogin_ConfirmPanel;
...
public class LoginPanel extends Window implements EventListener<Event>
{
    ...
    protected WmsLogin_ConfirmPanel pnlButtons; 
    ...
    public LoginPanel(Properties ctx, LoginWindow loginWindow)
    {
        ...
        applicationName = MSysConfig.getValue(MSysConfig.ZK_BROWSER_TITLE);
        initComponents();
        init();
...
//        td.appendChild(image);
    	...
    	Td td = new Td();
    	tr.appendChild(td);
    	welComeNote.setClass(ITheme.LOGIN_WELCOME_NOTE);
    	note.setClass(ITheme.LOGIN_NOTE);
    	td.appendChild(welComeNote);
    	td.appendChild(note);
    	td.setSclass(ITheme.LOGIN_WELCOME_NOTE_CONT);
    	...
    	td.setDynamicProperty("colspan", "2");
//    	td.setSclass(ITheme.LOGIN_FIELD_CLASS);
    	td.setSclass(ITheme.LOGIN_WELCOME_NOTE_SUBTITLE);
    	...
    	//create new table for labels and input fields 
    	
    	Table table2 = new Table();
    	table2.setId("grdLogin");
    	table2.setDynamicProperty("cellpadding", "0");
    	table2.setDynamicProperty("cellspacing", "5");
    	table2.setSclass(ITheme.LOGIN_FIELDS_CONT);
        ...
        table2.appendChild(tr);
    	td = new Td();
    	td.setSclass(ITheme.LOGIN_INPUT_FIELD);
    	tr.appendChild(td);
    	div = new Div();
//      div.setSclass("z-icon-user-o login-user");
    	div.setSclass("wms-user-icon login-user");
        td.appendChild(div);
    	...
    	tr.appendChild(td);
		div = new Div();
//        div.setSclass("z-icon-lock login-password");
		div.setSclass("wms-lock-icon login-password");
        td.appendChild(div);
        ...
        table2.appendChild(tr);
    	td = new Td();
    	td.setSclass(ITheme.LOGIN_INPUT_DROPDOWN);
    	tr.appendChild(td);
    	div = new Div();
//        div.setSclass("z-icon-globe login-language");
    	div.setSclass("wms-globe-icon login-language");
        td.appendChild(div);
...
//    	td.appendChild(chkSelectRole);
    	...
    	if (MSystem.isZKRememberUserAllowed()) {
            ...
            table2.appendChild(tr);
        	td = new Td();
        	td.setSclass(ITheme.LOGIN_REMEMBER_ME);
        	tr.appendChild(td);
        	td.appendChild(chkRememberMe);
        	lblForgotPwd.setSclass(ITheme.LOGIN_FORGOT_PWD);
        	td.appendChild(lblForgotPwd);
    	}
        ...
        pnlButtons = new WmsLogin_ConfirmPanel(false, false, false, false, false, false, true);
        pnlButtons.addActionListener(this);
        Button okBtn = pnlButtons.getButton(WmsLogin_ConfirmPanel.A_OK);
...
//        Button helpButton = pnlButtons.createButton(WmsLogin_ConfirmPanel.A_HELP);
...
//        ZKUpdateUtil.setWidth(pnlButtons, null);
        pnlButtons.getButton(WmsLogin_ConfirmPanel.A_OK).setSclass(ITheme.LOGIN_BUTTON_CLASS);
        ...
        div = new Div();
        div.setSclass(ITheme.LOGIN_VERSION_COPYRIGHT_CONT);
        div.appendChild (version);
        div.appendChild(copyRightLabel);
        this.appendChild(div);
	}

    private void initComponents()
    {
    	welComeNote = new Label();
    	welComeNote.setId("welComeNote");
    	welComeNote.setValue("Hi, Welcome to");
    	
    	note = new Label();
    	note.setId("note");
    	note.setValue(" "+applicationName+"!");
    	
    	subTitle = new Label();
    	subTitle.setId("subTitle");
    	subTitle.setValue("Enter your details  to sign in to your account. ");
    	
    	version =new Label();
    	version.setId("version");
    	version.setValue(Adempiere.getVersion());
    	
    	copyRightLabel=new Label();
    	copyRightLabel.setId("copyRightLabel");
    	copyRightLabel.setValue("@ "+applicationName+". All rights reserved.");
        ...
        lblForgotPwd =new Label();
        lblForgotPwd.setId("lblForgotPwd");
        lblForgotPwd.setValue("Forgot password");

        txtUserId = new Textbox();
        txtUserId.setId("txtUserId");
        txtUserId.setCols(25);
        txtUserId.setMaxlength(40);
        ZKUpdateUtil.setWidth(txtUserId, "100%");
...
//        txtPassword.setMaxlength(40);
        ZKUpdateUtil.setWidth(txtPassword, "100%");
        ...
        ZKUpdateUtil.setWidth(lstLanguage, "100%");
    ...
    public void onEvent(Event event)
    {
        ...
        if (event.getTarget().getId().equals(WmsLogin_ConfirmPanel.A_OK))
        {
            validateLogin();
        }
        else if (event.getTarget().getId().equals(WmsLogin_ConfirmPanel.A_HELP))
        {
            openLoginHelp();
        }
    ...
    private void languageChanged(String langName)
    {
		...
		if (email_login)
			...
			welComeNote.setValue(Msg.getMsg(language, "Hi, Welcome to "));
		note.setValue(Msg.getMsg(language, ""+applicationName+"!"));
		subTitle.setValue("Enter your details  to sign in to your account. ");
		lblForgotPwd.setValue("Forgot your password?");
		version.setValue("Version: "+Adempiere.getVersion());
		copyRightLabel.setValue("@ "+applicationName+". All rights reserved.");
		
		lblUserId.setValue(Msg.getMsg(language, "User Id"));
    	...
    	lblForgotPwd.setValue(Msg.getMsg(language, "Forgot your password?"));
    	chkSelectRole.setLabel(Msg.getMsg(language, "SelectRole"));
    	btnResetPassword.setLabel(Msg.getMsg(language, "ForgotMyPassword"));
    	lblLogin.setValue(Msg.getMsg(language, ""));
    	pnlButtons.getButton(WmsLogin_ConfirmPanel.A_OK).setLabel(Util.cleanAmp(Msg.getMsg(language,"Login")));
//    	pnlButtons.getButton(WmsLogin_ConfirmPanel.A_HELP).setLabel(Util.cleanAmp(Msg.getMsg(language, WmsLogin_ConfirmPanel.A_HELP)));
    }
    ...
    **/
    public void validateLogin()
    {
        ...
        else
        {
          ...
          // ** started: validation for Warepro CRM payment **//
    		String x_Forward_IP = Executions.getCurrent().getHeader("X-Forwarded-For");
    		if (x_Forward_IP == null) {
    			x_Forward_IP = Executions.getCurrent().getRemoteAddr();
    		}
    		if (!userId.equalsIgnoreCase("PaymentAdmin") && !validateUserPasskey()) {
    			String loginErrMsg = "Your subscription has expired. Please contact management for further assistance.";
    			logAuthFailure.log(x_Forward_IP, "/webui", userId, loginErrMsg);
            	...
            	Clients.clearBusy();
//           		throw new WrongValueException(loginErrMsg);
            	throw new AdempiereException(loginErrMsg);
    		}
    		// ** ended **//
    ...
    private static final String SECRET_KEY = "Warepro-Secretkey"; 

	public static boolean validateUserPasskey() {
		try {
			PreparedStatement pstm = null;
			ResultSet rs = null;
			try {
				String flag = MSysConfig.getValue("SUBSCRIPTION_VALIDATION");
				if(flag == null || flag.equalsIgnoreCase("FALSE"))
					return true;
				
				String query = "SELECT passKey\n" + "FROM adempiere.AD_Client\n"
						+ "WHERE name <> 'System' limit 1;\n" + "";
				pstm = DB.prepareStatement(query.toString(), null);
				rs = pstm.executeQuery();

				while (rs.next()) {
					String userPasskey = rs.getString("userPassKey");
					if (userPasskey == null)
						return false;
					// Parse the JWT and extract claims
					Claims claims = Jwts.parser().setSigningKey(SECRET_KEY) // Use the same secret key
							.parseClaimsJws(userPasskey) // Parse the token
							.getBody(); // Get the claims from the token

					// Extract the expiration date stored in the payload
					String dateString = claims.get("expDate", String.class); // Retrieve expiration date
					LocalDate validUptoDate = LocalDate.parse(dateString, DateTimeFormatter.ISO_DATE);

					// Get today's date
					LocalDate today = LocalDate.now();

					// Check if the expiration date is before today (expired)
					return !validUptoDate.isBefore(today); // Return true if not expired
				}
			} catch (Exception e) {
