...
public class HeaderPanel extends Panel implements EventListener<Event>
{
    ...
    protected void onCreate()
    {
		...
		boolean showHeaderSearch = MSysConfig.getBooleanValue("SHOW_HEADER_SEARCH", true);
		if (showHeaderSearch) {
			createPopupMenu();
		...
		boolean showHeaderMenu = MSysConfig.getBooleanValue("SHOW_HEADER_MENU", true);
		if (showHeaderMenu) {
			if (!showHeaderSearch)
				createPopupMenu();

			btnMenu = (LabelImageElement) getFellow("menuButton");
			btnMenu.setIconSclass("z-icon-th-large");
			btnMenu.setTooltiptext(Util.cleanAmp(Msg.getMsg(Env.getCtx(), "Menu")));
			
			if (!showHeaderSearch)
			    btnMenu.setStyle("margin-left: 100px;"); // Adjust the value as needed
		...
		if (showHeaderMenu || showHeaderSearch)
			SessionManager.getSessionApplication().getKeylistener().addEventListener(Events.ON_CTRL_KEY, this);
