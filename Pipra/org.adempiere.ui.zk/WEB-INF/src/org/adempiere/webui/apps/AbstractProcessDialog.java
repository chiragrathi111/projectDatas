...
public abstract class AbstractProcessDialog extends Window implements IProcessUI, EventListener<Event> {
	...
	/**
	 * Event to update status text of process dialog. Event data: status text
	 * message.
	 **/
	...
	/**
	 * Drop down editor for {@link X_AD_PInstance#NOTIFICATIONTYPE_AD_Reference_ID}
	 * list. For background job notification when {@link #runAsJobField} is checked.
	 */
	 ...
	 */
	protected boolean init(Properties ctx, int WindowNo, int AD_Process_ID, ProcessInfo pi, boolean autoStart,
	 ...
	 * @return true if init is ok.
	 */
	protected boolean init(Properties ctx, int WindowNo, int TabNo, int AD_Process_ID, ProcessInfo pi,
			boolean autoStart, boolean isDisposeOnComplete) {
		m_ctx = ctx;
		m_WindowNo = WindowNo;
		m_TabNo = TabNo;
	...
	/**
	 * Layout {@link #mainParameterLayout}, {@link #topParameterLayout} and
	 * {@link #bottomParameterLayout}.
	 */
	 ...
	 * {@link #parameterPanel}, {@link #runAsJobField} and
	 * {@link #notificationTypeField}.
	 * 
	 * @param parent
	 */
	protected void inputParameterLayout(HtmlBasedComponent parent) {
		...
		if (MSysConfig.getBooleanValue(MSysConfig.BACKGROUND_JOB_ALLOWED, false)) {
			...
			MLookupInfo info = MLookupFactory.getLookup_List(Env.getLanguage(m_ctx),
					MPInstance.NOTIFICATIONTYPE_AD_Reference_ID);
			notificationTypeField = new WTableDirEditor(MPInstance.COLUMNNAME_NotificationType, true, false, true,
					new MLookup(info, 0));
	...
	/**
	 * Layout content of {@link #bottomParameterLayout}. Report option, save
	 * parameter and action buttons.
	 * 
	...
	/**
	 * @return true if current process is with IsReport=Y AND JasperReport Is Not
	 *         NULL.
	 */
	...
	protected void savePrameterLayout(HtmlBasedComponent bottomParameterLayout) {
		...
		savePrameterLayout.appendChild(div);
	...
	protected void querySaved() {
		// user query
		savedParams = MPInstance.get(Env.getCtx(), getAD_Process_ID(),
				Env.getContextAsInt(Env.getCtx(), Env.AD_USER_ID));
	...
	protected void buttonLayout(HtmlBasedComponent bottomParameterLayout) {
		...
		bOK = ButtonFactory.createNamedButton(ConfirmPanel.A_OK, true, false);
		bOK.setId("Ok");
		bOK.addEventListener(Events.ON_CLICK, this);
		confParaPanel.appendChild(bOK);
		confParaPanel.appendChild(new Space());

		bCancel = ButtonFactory.createNamedButton(ConfirmPanel.A_CANCEL, true, false);
	...
	private void listPrintFormat(MClient client) {
		...
		try {
			...
			if (pr.getAD_ReportView_ID() > 0 && MSysConfig.getBooleanValue(
					MSysConfig.ZK_REPORT_ONLY_PRINTFORMAT_LINKEDTO_REPORTVIEW, false, client.getAD_Client_ID())) {
				if (valCode.length() > 0)
					valCode.append(" AND ");
			...
			Lookup lookup = MLookupFactory.get(Env.getCtx(), m_WindowNo, AD_Column_ID, DisplayType.TableDir,
					Env.getLanguage(Env.getCtx()), "AD_PrintFormat_ID", 0, false, valCode.toString());
	...
	protected MPInstance getLastRun() {
		...
		return new Query(Env.getCtx(), MPInstance.Table_Name, where, null).setOnlyActiveRecords(true).setClient_ID()
	...
	/**
	 * Set value for {@link #fPrintFormat}, {@link #fLanguageType},
	 * {@link #freportType} and {@link #chbIsSummary} from instance.
	 * 
	 * @param instance
	 */
	...
	/**
	 * Update process info ({@link ProcessInfo}) with selected report options
	 * ({@link #freportType}, {@link #fPrintFormat}, {@link #fLanguageType} and
	 * {@link #chbIsSummary}).
	 */
	...
	/**
	 * Auto start process upon instantiation of process dialog. Delegate to
	 * {@link #startProcess0()}.
	 */
	...
	public void onEvent(Event event) {
		...
		if (component == runAsJobField && event.getName().equals(Events.ON_CHECK)) {
		...
		else if (event.getTarget().equals(bSave) || event.getTarget().equals(bDelete)
				|| event.getTarget().equals(fSavedName)) {
			...
			if (isBackgroundJob() && getNotificationType() == null)
				throw new WrongValueException(notificationTypeField.getComponent(),
						Msg.getMsg(m_ctx, "FillMandatory") + notificationTypeLabel.getValue());
			saveReportOption();
		}
	}

	/**
	 * Save process parameters and report options. Set MPInstance.Name = saveName.
	 * 
	 * @param saveName
	 */
		...
		if (fSavedName.getSelectedIndex() > -1 && savedParams != null) {
			for (int i = 0; i < savedParams.size(); i++) {
				if (savedParams.get(i).getName().equals(saveName)) {
					getProcessInfo().setAD_PInstance_ID(savedParams.get(i).getAD_PInstance_ID());
	...
	/**
	 * Save report options (output type, print format, language and IsSummary) to
	 * instance.
	 * 
	...
	/**
	 * Run process. Delegate to {@link #startProcess0()}.
	 */
	...
	public void runProcess() {
		...
		future = Adempiere.getThreadPoolExecutor()
				.submit(new DesktopRunnable(new ProcessDialogRunnable(null), getDesktop()));
	}

	/**
	 * Run process as background job (runBackgroundJob event echo from
	 * {@link #startProcess0()}). <br/>
	 * The different with {@link #runProcess()} is this method doesn't wait for
	...
	public void runBackgroundJob() {
		...
		try {
			...
			int count = new Query(m_ctx, MPInstance.Table_Name,
					"Coalesce(AD_User_ID,0)=? AND IsProcessing='Y' AND IsRunAsJob='Y' ", null)
					.setOnlyActiveRecords(true).setClient_ID().setParameters(AD_User_ID).count();
			...
			count = new Query(m_ctx, MPInstance.Table_Name, "IsProcessing='Y' AND IsRunAsJob='Y' ", null)
					.setOnlyActiveRecords(true).setClient_ID().count();
			...
			Adempiere.getThreadPoolExecutor().schedule(new BackgroundJobRunnable(getCtx()), 1000,
	...
	/**
	 * Unlock dialog upon completion of process (or upon submission of job if
	 * process is running as background job).
	 */
	@Override
	...
	/**
	 * Close process in progress dialog and update UI with the result of process
	 * execution.
	 */
	...
	/**
	 * Runnable to run process in background thread. Notify process dialog with
	 * {@link AbstractProcessDialog#ON_COMPLETE_EVENT} event.
	 */
	private class ProcessDialogRunnable extends ZkContextRunnable {
		...
		protected void doRun() {
			...
			try {
				...
				Executions.schedule(getDesktop(), AbstractProcessDialog.this,
						new Event(ON_COMPLETE_EVENT, AbstractProcessDialog.this, null));
			}
		}
	}

	/**
	 * Runnable to run process as background job. Send email or notice notification
	 * to user upon completion of job.
	 */
	private class BackgroundJobRunnable implements Runnable {
		...
		private void doRun() {
			...
			boolean sendEmail = notificationType.equals(MPInstance.NOTIFICATIONTYPE_EMail)
					|| notificationType.equals(MPInstance.NOTIFICATIONTYPE_EMailPlusNotice);
			boolean createNotice = notificationType.equals(MPInstance.NOTIFICATIONTYPE_Notice)
					|| notificationType.equals(MPInstance.NOTIFICATIONTYPE_EMailPlusNotice);
			...
			try {
				...
				if (!m_pi.isError()) {
					boolean isReport = (process.isReport() || process.getAD_ReportView_ID() > 0
							|| process.getJasperReport() != null || process.getAD_PrintFormat_ID() > 0);
				...
				if (sendEmail) {
					...
					client.sendEMailAttachments(AD_User_ID,
							process.get_Translation("Name", Env.getAD_Language(Env.getCtx())),
							m_pi.getSummary() + " " + m_pi.getLogInfo(), getDownloadFiles());
				}
				...
				if (createNotice) {
					...
					if (attachment != null)
						attachment.saveEx();
					MPInstanceLog il = instance.addLog(null, 0, null,
							Msg.parseTranslation(m_ctx, "@Created@ @AD_Note_ID@ " + note.getAD_Note_ID()),
