...
public class DPPurchase extends DashboardPanel implements EventListener<Event> {
		...
		try {
			...
			if (wareHouseId == 0 && productId == 0) {
				sql = "SELECT count(distinct co.c_order_id) as orderCount, col.count as orderLineCount, sum(col.qtyInvoiced) as qnty, \n"
						+ "sum(co.grandtotal) as totalamount FROM C_order co\n"
						+ "join c_orderline col on col.c_order_id = co.c_order_id \n"
						+ "join m_warehouse mw on mw.m_warehouse_id = co.m_warehouse_id\n"
						+ "where co.issotrx='N' and co.ad_client_id=" + clientId + " and col.qtyInvoiced !=0;";
			} else if (wareHouseId != 0 && productId == 0) {
				sql = "SELECT count(distinct co.c_order_id) as orderCount, col.count as orderLineCount, sum(col.qtyInvoiced) as qnty, \n"
						+ "sum(co.grandtotal) as totalamount FROM C_order co\n"
						+ "join c_orderline col on col.c_order_id = co.c_order_id \n"
						+ "join m_warehouse mw on mw.m_warehouse_id = co.m_warehouse_id\n"
						+ "where co.issotrx='N' and mw.name='" + warehouseDefaultName + "' and co.ad_client_id="
				...
				sql = "SELECT count(distinct co.c_order_id) as orderCount, col.count as orderLineCount, sum(col.qtyInvoiced) as qnty,\n"
						...
						+ "join c_orderline col on col.c_order_id = co.c_order_id \n"
						+ "join m_warehouse mw on mw.m_warehouse_id = co.m_warehouse_id \n"
						+ "where co.issotrx='N' and col.m_product_id = "+productId+" and co.ad_client_id="+clientId+" and col.qtyInvoiced !=0;";
			}
