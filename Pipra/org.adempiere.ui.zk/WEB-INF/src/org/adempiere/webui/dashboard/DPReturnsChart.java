...
public class DPReturnsChart extends DashboardPanel implements EventListener<Event> {
	...
	private void initLineModel() {
		...
		dataList = new ArrayList<>(Arrays.asList(
				monthSalesList.get(currentMonth.getValue()-1) != 0 ? (monthReturnsList.get(currentMonth.getValue()-1) * 100)
						/ monthSalesList.get(currentMonth.getValue()-1) : 0,
				monthSalesList.get(currentMonth.minus(1).getValue()-1) != 0
						? (monthReturnsList.get(currentMonth.minus(1).getValue()-1) * 100)
								/ monthSalesList.get(currentMonth.minus(1).getValue()-1)
						: 0,
				monthSalesList.get(currentMonth.minus(2).getValue()-1) != 0
						? (monthReturnsList.get(currentMonth.minus(2).getValue()-1) * 100)
								/ monthSalesList.get(currentMonth.minus(2).getValue()-1)
						: 0,
				monthSalesList.get(currentMonth.minus(3).getValue()-1) != 0
						? (monthReturnsList.get(currentMonth.minus(3).getValue()-1) * 100)
								/ monthSalesList.get(currentMonth.minus(3).getValue()-1)
						: 0,
				monthSalesList.get(currentMonth.minus(4).getValue()-1) != 0
						? (monthReturnsList.get(currentMonth.minus(4).getValue()-1) * 100)
								/ monthSalesList.get(currentMonth.minus(4).getValue()-1)
						: 0,
				monthSalesList.get(currentMonth.minus(5).getValue()-1) != 0
						? (monthReturnsList.get(currentMonth.minus(5).getValue()-1) * 100)
								/ monthSalesList.get(currentMonth.minus(5).getValue()-1)
						: 0));
		...
		if (data == null || data.isEmpty()) {
			...
			data.addDataset(Dataset.Builder.init().fill(false).borderColor("#2563EB").label("Return rate")
					.backgroundColor("#A0D7E7").data(dataList).build());
			data.getDatasets().remove(0);
		} else {
			Dataset dataset = data.getDatasets().get(0);
			dataset.setData(dataList);
			data.getDatasets().set(0, dataset);
		}
	}
