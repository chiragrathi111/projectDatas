...
//					
//					if (DPWarehouseSelection.getDpName(dp).equalsIgnoreCase("DPWarehouseSelection") && dp.getDesktop() == desktop.get())
//						flag = true;
//				}
//				if (flag) {
//					DashboardRunnable dashboardRunnable = DPWarehouseSelection.refreshDashboardPanels(desktop.get());
//					dashboardRunnable.refreshDashboard(true);
//				} else
					refreshDashboard(false);  //update if true don't screen update
			} catch (Exception e) {
	...
	public void refreshDashboard(boolean pooling)
	{
		...
		try
		{
	    	...
	    	for(int i = 0; i < dashboardPanels.size(); i++)
	    	{
	    		...
	    		final DashboardPanel dpanel = dashboardPanels.get(i);   //all screen referesh
