...
import org.adempiere.webui.util.XSSUtil;
import org.adempiere.webui.util.HTMLEncoder;
...
public class WStringEditor extends WEditor implements ContextMenuListener
{
	...
	public void onEvent(Event event)
    {
    	...
    	if (Events.ON_CHANGE.equals(event.getName()) || Events.ON_OK.equals(event.getName()) || isStartEdit)
    	{
	        ...
	        // Validate for XSS before any processing
	        if (!isStartEdit && newValue != null && newValue.length() > 0 && XSSUtil.containsXSS(newValue)) {
	        	String errorMsg = XSSUtil.getErrorMessage(getColumnName());
	        	Messagebox.show(errorMsg, "Security Error", Messagebox.OK, Messagebox.ERROR);
	        	Events.postEvent(new Event(Events.ON_CHANGE, getComponent(), oldValue));
    ...
    public void setValue(Object value)
    {
        if (value != null)
        {
            // Encode output to prevent XSS in display
            String encodedValue = HTMLEncoder.encode(value.toString());
            getComponent().setValue(encodedValue);
        }
        else
        {
	...
	/**
	 * Check if the display type represents a string field that should be validated for XSS
	 * 
	 * @param displayType The display type from GridField
	 * @return true if it's a string field that should be validated
	 */
	private boolean isStringField(int displayType) {
		return displayType == DisplayType.String ||
