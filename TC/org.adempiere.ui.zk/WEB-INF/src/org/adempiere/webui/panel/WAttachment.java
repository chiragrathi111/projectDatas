...
public class WAttachment extends Window implements EventListener<Event>
{
	...
	public static final String ATTACHMENT_ALLOW_FILE_EXTENSIONS = "ATTACHMENT_ALLOW_FILE_EXTENSIONS";
	public static final String ATTACHMENT_ALLOW_FILE_EXTENSIONS_ERROR_MSG = "ATTACHMENT_ALLOW_FILE_EXTENSIONS_ERROR_MSG";

	public static final String ATTACHMENT_RESTRICT_FILE_SIZE = "ATTACHMENT_RESTRICT_FILE_SIZE";
	public static final String ATTACHMENT_RESTRICT_FILE_SIZE_ERROR_MSG = "ATTACHMENT_RESTRICT_FILE_SIZE_ERROR_MSG";
...
//		bLoad.setAttribute("org.zkoss.zul.image.preload", Boolean.TRUE);
	...
	private void processUploadMedia(Media media) {
		...
		if (media != null && ((media.isBinary() && media.getByteData().length > 0)
				|| (!media.isBinary() && media.getStringData().length() > 0))) {
//				pdfViewer.setContent(media);
		...
		String defaultAllowedExtensions = "";
		String defaultExtensionErrorMsg = "Only the allowed file types can be uploaded.";

		long defaultMaxSizeMB = 100;
		long defaultMaxSize = defaultMaxSizeMB * 1024 * 1024;
		String defaultSizeErrorMsg = "Maximum file size allowed is " + defaultMaxSizeMB + " MB.";

		String defaultContentErrorMsg = "The file content is invalid or does not match the file extension.";


		String allowedFileExtensions = MSysConfig.getValue(ATTACHMENT_ALLOW_FILE_EXTENSIONS, defaultAllowedExtensions);
		String extensionErrorMsg = MSysConfig.getValue(ATTACHMENT_ALLOW_FILE_EXTENSIONS_ERROR_MSG,
				defaultExtensionErrorMsg);

		String maxSizeStr = MSysConfig.getValue(ATTACHMENT_RESTRICT_FILE_SIZE, String.valueOf(defaultMaxSize));
		long MAX_FILE_SIZE_BYTES;
		try {
			MAX_FILE_SIZE_BYTES = Long.parseLong(maxSizeStr);
		} catch (NumberFormatException e) {
			MAX_FILE_SIZE_BYTES = defaultMaxSize;
		}
		String sizeErrorMsg = MSysConfig.getValue(ATTACHMENT_RESTRICT_FILE_SIZE_ERROR_MSG, defaultSizeErrorMsg);


		if (!allowedFileExtensions.isEmpty()) {

			String[] allowedExtensionsArray = allowedFileExtensions.toLowerCase().split(",");

			boolean extensionAllowed = false;
			for (String allowedExt : allowedExtensionsArray) {
				if (allowedExt.trim().equals(fileExtension)) {
			...
			if (!extensionAllowed) {
				showAttachmentError("Invalid File Type", extensionErrorMsg);
				return;
			}
		}

		long fileSize = media.getByteData() != null ? media.getByteData().length : 0;

		if (fileSize > MAX_FILE_SIZE_BYTES) {
			String finalSizeErrorMsg = sizeErrorMsg.replace("{MAX_SIZE_MB}",
					String.valueOf(MAX_FILE_SIZE_BYTES / 1024 / 1024));
			finalSizeErrorMsg = finalSizeErrorMsg + " (Current: " + (fileSize / 1024 / 1024) + " MB)";

			showAttachmentError("File Size Exceeded", finalSizeErrorMsg);
			return;
		}

		byte[] fileData = media.getByteData();
		if (fileData == null || fileData.length == 0) {
			showAttachmentError("Upload Error", "Could not read file data.");
			return;
		}

		if (!isFileSignatureValid(fileData, fileExtension, allowedFileExtensions)) {
		    showAttachmentError("Security Alert", defaultContentErrorMsg);
		    return;
		}
	...
	private boolean isFileSignatureValid(byte[] data, String fileExtension, String allowedExtensionsConfig) {
	    
	    String config = allowedExtensionsConfig.toLowerCase();

	    if (!config.contains("png") && !config.contains("jpg") && !config.contains("jpeg")) {
	...
	private void showAttachmentError(String title, String message) {
