...
//    private HttpServletRequest httpServletRequest;
    ...
    int portNumber = req.getServerPort();        // port
    ...
    public LoginPanel(Properties ctx, LoginWindow loginWindow)
    {
        ...
        applicationTitle = MSysConfig.getValue("ZK_APP_NAME");
...
//        	    + ((portNumber == 80 || portNumber == 443) ? "" : ":" + portNumber)
        	    + contextPath;

		String publicBaseURL = baseURL.replaceFirst("/webui.*", "/forgot-password/");
//		System.out.println(publicBaseURL);
        
        if (MSysConfig.getBooleanValue(MSysConfig.LOGIN_SHOW_RESETPASSWORD, true)) {

//        	btnResetPassword.setHref("https://tc.warepro.in/forgot-password/");
        	btnResetPassword.setHref(publicBaseURL);
            btnResetPassword.setSclass(ITheme.LOGIN_FORGOT_PWD);
...
//    	welComeNote.setValue("Tissue Culture");
    	
    	note = new Label();
    	note.setId("note");
//    	note.setValue(" "+applicationName+"!");
    	note.setValue(applicationTitle);
...
//			welComeNote.setValue(Msg.getMsg(language, "Hi, Welcome to "));
		note.setValue(Msg.getMsg(language, applicationTitle));
		subTitle.setValue("Enter your details  to sign in to your account. ");
