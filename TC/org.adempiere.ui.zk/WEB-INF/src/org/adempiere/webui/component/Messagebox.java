...
//	private Image img = new Image();
	...
	private void init() {
		...
		btnOk = ButtonFactory.createNamedButton(ConfirmPanel.A_OK, true, false);
		btnOk.setId("btnOk");
		btnOk.addEventListener(Events.ON_CLICK, this);
		this.appendChild(btnOk);

		btnCancel = ButtonFactory.createNamedButton(ConfirmPanel.A_CANCEL, true, false);
		...
		btnYes = ButtonFactory.createButton(ButtonFactory.isWithText() ? yesLabel : null,
				ButtonFactory.isWithImage()
						? (ButtonFactory.isWithText() ? ThemeManager.getThemeResource("images/Ok16.png")
								: ThemeManager.getThemeResource("images/Ok24.png"))
						: null,
		...
		btnNo = ButtonFactory.createButton(ButtonFactory.isWithText() ? noLabel : null,
				ButtonFactory.isWithImage()
						? (ButtonFactory.isWithText() ? ThemeManager.getThemeResource("images/Cancel16.png")
								: ThemeManager.getThemeResource("images/Cancel24.png"))
						: null,
...
//		north.appendChild(pnlImage);
	 ...
	 * @param message
	 * @param title
	 * @param buttons buttons to be shown in the dialog. use the | operator to
	 *                combine multiple button constant.
	 ...
	 * @param message
	 * @param title
	 * @param buttons          buttons to be shown in the dialog. use the | operator
	 *                         to combine multiple button constant.
	 * @param icon             image URL for message box icon
	 ...
	 * @return button constant for button press by user
	 */
	public int show(String message, String title, int buttons, String icon, WEditor editor, boolean isInputMandatory,
			Callback<?> callback, boolean modal) {
		this.msg = message;
		this.imgSrc = icon;
	 ...
	 * @return button constant for button press by user
	 */
	public static int showDialog(String message, String title, int buttons, String icon, Callback<?> callback,
			boolean modal) {
	 ...
	 * @return button constant for button press by user
	 */
	public static int showDialog(String message, String title, int buttons, String icon, WEditor editor,
			Callback<?> callback, boolean modal) {
	 ...
	 */
	public static int showDialog(String message, String title, int buttons, String icon, WEditor editor,
			boolean isInputMandatory, Callback<?> callback, boolean modal) {
	...
	private void validateOnClose() {
		...
		if ((returnValue == CANCEL) || !isInputMandatory
				|| (isInputMandatory && !Util.isEmpty(String.valueOf(inputField.getValue()))))

			// if Valid Input is defined, don't close on OK until user types the Valid Input
			if (!Util.isEmpty(inputField.getValidInput())) {

				if ((returnValue == CANCEL)
						|| (inputField.isValid(String.valueOf(inputField.getValue())) && returnValue == OK)) {
					close();
				} else {
					isExceptionThrown = true;
					returnValue = 0;
					throw new WrongValueException(inputField.getComponent(),
							Msg.getMsg(Env.getCtx(), "ValueNotCorrect"));
				}
			} else {
				close();
			}
