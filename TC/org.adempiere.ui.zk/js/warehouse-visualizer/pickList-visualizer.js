...
function loadPickListGrid(x, y) {


	draw.text('Pick List').move(x, y - 30).font({ fill: '#000000', family: 'Inconsolata' });
	...
	for (let i = 0; i < columns.length; i++) {
		draw.text(columns[i]).move(x + 30, y).font({ fill: '#000000', family: 'Inconsolata' });
		x = x + 90;
	}
	...
	for (let i = 0; i < 3; i++) {
		for (let j = 0; j < 3; j++) {
			...
			draw.text('- 0%').move(x + 30, y +10).font({ fill: '#000000', family: 'Inconsolata' });
			x = x + 30;
		}
		x = x - 90;
		y = y + 30;
